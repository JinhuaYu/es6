...
var makeIssue = function(){
  ...
  if (date.getTime()-first_issue_date.getTime()>0 && date.getTime()-end_issue_date.getTime()<0) {
    ...
    var end_date = new Date(cur_issue_date.getTime()+1000*60*h)
    end_time = end_date.getTime()
    cur_issue = [end_date.getFullYear(), ('0'+end_date.getMonth()+1).slice(-2), ('0'+end_date.getDate()).slice(-2), ('0'+end_date.getDate()).slice(-2), ('0'+h).slice(-2)].join('')
  } else {
    // 截至销售
